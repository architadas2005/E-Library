...
from application.models import Book, db,User, Section, BookRequest, Feedback, DailyVisit, Rating
        ...
        try:
            ...
            if 'image' in request.files:
                ...
                if file:
                    ...
                    file.save("static/book_covers/" + filename)
                    file.close()
...
class RequestBooks(Resource):
    @auth_required("token")
    def get(self, book_id):
        # Check the number of active requests
        active_requests_count = BookRequest.query.filter_by(
        ...
        if active_requests_count >= 5:
            return {"message": "You can only request a maximum of 5 e-books at a time"}, 400
        ...
        db.session.add(req)
        db.session.commit()
        return {"message": "Book request successful"}, 201
...
#         parser.add_argument('min_rating', type=float, help="Minimum Rating", required=False)
#         args = parser.parse_args()
        
#         search_value = args.get('search')
#         search = "%{}%".format(search_value)
        
#         # Get the minimum rating if provided
#         min_rating = args.get('min_rating')
        
#         sections = Section.query.filter(Section.section_name.like(search)).all()
#         books_query = Book.query.filter(Book.title.like(search) | Book.author.like(search))
        
#         if min_rating is not None:
#             books_query = books_query.join(Rating).group_by(Book.book_id).having(db.func.avg(Rating.rating) >= min_rating)
...
class LibrarianReport(Resource):
    ...
    def get(self):
        ...
        for book in books:
            ...
            issued_requests = BookRequest.query.filter_by(book_id=book.book_id, is_approved=True, is_rejected=False, is_returned=False, is_revoked=False).all()
            issued_count = len(issued_requests)
            issued_counts[book.title] = issued_count

            # Calculate average rating
            ratings = Rating.query.filter_by(book_id=book.book_id).all()
            if ratings:
                average_rating = sum(r.rating for r in ratings) / len(ratings)
                book_ratings[book.title] = average_rating
...
class RateBook(Resource):
    ...
    def post(self, book_id):
        parser = reqparse.RequestParser()
        parser.add_argument('rating', type=int, help="Rating is required and should be an integer", required=True)
        args = parser.parse_args()

        # Check if the rating is within the valid range
        if not (1 <= args['rating'] <= 5):
            return {"message": "Rating should be between 1 and 5"}, 400
        ...
        if existing_rating:
            ...
            new_rating = Rating(user_id=current_user.id, book_id=book_id, rating=args['rating'])
            db.session.add(new_rating)

        db.session.commit()
        return {"message": "Rating submitted successfully"}, 201

api.add_resource(RateBook, '/rate-book/<int:book_id>')
