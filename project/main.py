...
def automated_tasks(sender, **kwargs):
    ...
    # )
    sender.add_periodic_task(
        crontab(hour=21, minute=23),  # Daily at 6:30 AM
        daily_reminder.s(),
    )
    ...
    # )
    sender.add_periodic_task(
        crontab(hour=21, minute=24),  # Daily at 12:01 AM
        mark_overdue_requests_as_revoked.s(),
    )
    ...
    # )
    sender.add_periodic_task(
        crontab(day_of_month=7, hour=21, minute=25),  # Monthly on the 1st day at 12:01 AM
        send_monthly_report.s(),
    )
