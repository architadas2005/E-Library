export default ({
    ...
    methods: {
        ...
        submitRating() {
            fetch("/api/rate-book/" + this.bookInfo.book_id, {
                method: "POST",
                headers: {
                    'Authentication-Token': localStorage.getItem('auth-token'),
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({rating: this.new_rating})
            }).then(() => {
                this.new_rating = 0;
                this.getBooksDetails();
            })
        ...
        getBooksDetails() {
            ...
            if (res.rating) {
                this.bookInfo.rating = res.rating;
            }
        },
        playPause() {
    ...
    template: `
    <div class="px-3 pt-3 pb-5" style="background-color: white;">
        <div class="clearfix my-2">
            <div class="float-start">
                ...
                <button class="btn btn-sm btn-info" v-if="!bookInfo.wrote_review && role=='member'" data-bs-toggle="modal" data-bs-target="#bookReviewModal">Write a Review</button>
           
            </div>    
            <div class="float-end">
                <img height="150" :src="'static/book_covers/'+bookInfo.image" alt="Book Image" />
            </div>    
        </div>
        <h5>Content : </h5>
        <hr>
<!--        <button class="btn btn-primary" @click="playPause">Play/Pause</button>-->
        ...
        <div v-if="role=='member'">
            ...
            <select v-model="new_rating" id="rating">
                <option v-for="n in 5" :key="n" :value="n">{{n}}</option>
            </select>
            <button class="btn btn-sm btn-primary" @click="submitRating">Submit Rating</button>
        </div>
        <div v-if="bookInfo.rating">
            <h5>Average Rating: {{bookInfo.rating}}</h5>
        </div>
    </div>
